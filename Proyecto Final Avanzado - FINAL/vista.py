...
class Botones():
    def __init__(self, root, objeto_botones, a_val, b_val, c_val, d_val, e_val, tree):
        self.root = root
        self.objeto_server = todo_servidor()
        ...
        # Decorado servidor
        row_offset = 1
        frame_servidor = Frame(self.root, bg="red")
        frame_servidor.grid(row=row_offset, column=2, columnspan=2, padx=10, pady=(5, 0))

        # Agregar el texto "Servidor" en el lado derecho
        label_servidor = Label(frame_servidor, text="Servidor", bg="red", fg="white", font=('Arial', 10, 'bold'))
        label_servidor.grid(row=0, column=1, padx=(100, 100), pady=(0,0), sticky="W")

             #PRENDIDO/ENCENDIDO SERVIDOR
#_________________________ 

        self.boton_prendido= self.crear_boton("Prender", command=lambda:self.objeto_server.try_connection())
        self.boton_prendido.grid(row=row_offset, column=2, padx=(0, 0), pady=(0, 5))

        self.boton_apagar= self.crear_boton("Apagar", command=lambda:self.objeto_server.stop_server())
        self.boton_apagar.grid(row=row_offset, column=3, padx=(0, 0), pady=(0, 5))

#________________________________________
     #DECLARACION DE LOS DISTINTOS BOTONES FUNCIONALES
#_________________________
...
#_________________________        
        ...
        self.boton_afip.grid(row=1, column=3, rowspan=5, padx=(0, 10), pady=(0, 10), sticky="W")
...
#configuracion general de todos los botones
